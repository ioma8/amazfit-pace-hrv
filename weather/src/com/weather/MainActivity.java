...
public class MainActivity extends ProbeActivity {
    ...
    protected void onExitCleanup() {
        ...
        if (wifi != null && wifi.isWifiEnabled()) {
            android.util.Log.i("Weather", "paused: disabling wifi");
        ...
        executor.submit(new Runnable() {
            @Override public void run() {
                try {
                    ...
                    android.util.Log.i("Weather", "download ok: " + forecast[0].entries.size()
                            + "+" + forecast[1].entries.size() + " hours");
                    runOnUiThread(new Runnable() {
                        @Override public void run() { showForecast(forecast[0], forecast[1], "Updated"); }
                    });
                } catch (final Exception error) {
                    android.util.Log.e("Weather", "download failed", error);
                    runOnUiThread(new Runnable() {
                        @Override public void run() {
