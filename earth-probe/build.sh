#!/bin/bash
# Build earth-probe APK (spinning Earth with live day/night terminator). Produces aligned.apk (signed).
set -e
cd "$(dirname "$0")"
...
rm -rf obj dexout
mkdir -p obj dexout
"$BT/aapt" package -f -M AndroidManifest.xml -S res -I "$AJ" -F unsigned.apk -J obj
javac --release 8 -classpath "$AJ" -d obj $(find src -name '*.java') obj/R.java
